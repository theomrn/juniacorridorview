...
import { createVisitorType, updateVisitorType, deleteVisitorType, getVisitorTypes } from "../api/AxiosVisitorType";
import { FaChevronDown, FaChevronUp, FaPen, FaTrash, FaPlus, FaArrowLeft } from "react-icons/fa";
import { toast } from "sonner";
import '../style/AdminVisitorType.css';

export default function AdminVisitorType() {
  ...
  const startEdit = (visitorType) => {
    ...
    setEditFormData({
      name_visitor_type: visitorType.name_visitor_type
    });
  };
  ...
  const handleSaveEdit = async (id_visitor_type) => {
    if (!editFormData.name_visitor_type.trim()) {
      toast.error('Le nom du type de visiteur est requis');
      return;
    }

    try {
      await updateVisitorType(id_visitor_type, editFormData.name_visitor_type);
  ...
  return (
    <div className="admin-visitor-type-container">
      <div className="admin-visitor-type-back-button" style={{position: "fixed", left: "20px", top: "80px", zIndex: 40}}>
        <button
      ...
      <div className="admin-visitor-type-header">
        <h1 className="text-3xl font-title font-bold text-junia-purple">Types de Visiteurs</h1>
        <button
          onClick={() => setShowNewVisitorTypeForm(!showNewVisitorTypeForm)}
          className="button-type font-title font-bold flex items-center gap-2"
          style={{ backgroundColor: '#f06b42', color: 'white' }}
        >
          <FaPlus /> Ajouter un type
        </button>
      </div>

      {/* Formulaire d'ajout rapide */}
      {showNewVisitorTypeForm && (
        <div className="admin-visitor-type-new-form">
          <form onSubmit={handleAddNewVisitorType}>
            <div className="form-group">
              ...
              <input
                ...
                placeholder="Ex: Étudiant, Visiteur, Professionnel"
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '2rem', color: '#666' }}>
          <p>Aucun type de visiteur trouvé. Cliquez sur "Ajouter un type" pour en créer un.</p>
        </div>
      ) : (
        <div className="admin-visitor-type-list">
          {visitorTypes.map((visitorType) => (
            <div key={visitorType.id_visitor_type} className="visitor-type-item">
              <div
                className="visitor-type-header"
                onClick={() => toggleExpand(visitorType.id_visitor_type)}
              >
                <div className="visitor-type-info">
                  <span className="visitor-type-name font-title font-bold">
              ...
              {expandedId === visitorType.id_visitor_type && (
                <div className="visitor-type-content">
                    ...
                    <form className="edit-form" onSubmit={(e) => {
                      ...
                      <div className="form-group">
                        ...
                        <input
                          ...
                          value={editFormData.name_visitor_type}
