...
const ModalAddEditFloors = ({
    ...
    const handleNewFloorSubmit = (e) => {
        ...
        if(!floorName || floorName.trim() === "") {
            toast.error(t('pleaseEnterFloorName'));
            return;
        }
        if(!file) {
        ...
        if(editMode) {
            ...
            api.updateFloor(formData)
                .then(() => {
                    toast.success(t('floorModifiedSuccess'));
                    toggle();
                    reload();
                })
                .catch((error) => {
                ...
                <div className="modal">
                    <div className="modal-content">
                        <div className="flex justify-between items-center pb-4">
                            <div className="text-3xl font-bold font-title text-center">{editMode ? t('editFloor') : t('addNewFloor')}</div>
                            <span className="close items-center" onClick={toggle}>&times;</span>
                        </div>
                        <form onSubmit={handleNewFloorSubmit}>
                            <div className="flex items-center gap-4">
                                <div className="fonts-title text-junia-purple font-bold w-1/3">{t('floorNameLabel')}</div>
                                <input
                                    type="text"
                                    name="name"
                            ...
                            <div className="flex items-center gap-4">
                                <label className="block font-bold text-junia-purple w-1/3">{t('planLabel')}</label>
                                <div className="w-2/3">
